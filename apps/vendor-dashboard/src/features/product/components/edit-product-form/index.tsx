    ...
    <div {...props}>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          />
          {isSuccess && (
            <FormResponse title="Success" variant="success" description="Product has been updated successfully." />
          )}
          {isError && (
            <FormResponse
              title="Error"
              variant="destructive"
              description={normalizeServerError(error, 'An error occurred while updating the product.')}
            />
          )}

...
import { db } from "@/db";
import { resetPasswordTokens, twoFactorConfirmations, twoFactorTokens, verificationTokens } from "@/db/schemas";
...
const deleteVerificationTokenByToken = async (token: string, trx: typeof db = db) => {
	await trx.delete(verificationTokens).where(eq(verificationTokens.token, token));
};

const deleteResetPasswordTokenByToken = async (token: string, trx: typeof db = db) => {
	await trx.delete(resetPasswordTokens).where(eq(resetPasswordTokens.token, token));
};

const deleteTwoFactorTokenByToken = async (token: string, trx: typeof db = db) => {
...
const createResetPasswordToken = async (userId: string, trx: typeof db = db) => {
	const token = await generateToken(tokenConfig.resetPasswordToken.length);
	const expires = new Date(Date.now() + tokenConfig.resetPasswordToken.maxAgeInSeconds);
